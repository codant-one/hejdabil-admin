    ...
    public function store(Request $request)
    {
        try {
            ...
            SupplierActivity::createActivity([
                ...
                'title' => 'Faktura #'.$billing->invoice_id.' - tillagd',
    ...
    public function reminder($id)
    {
        try {
            ...
            SupplierActivity::createActivity([
                ...
                'title' => 'Påminnelse #'.$billing->invoice_id.' - skickad',
                'description' => 'En påminnelse har skickats till kunden.',
                'icon' => 'custom-facture',
                'route' => '/dashboard/admin/billings/'.$billing->id,
                'metadata' => json_encode([
                    'billing_id' => $billing->id,
    ...
    public function downloadZip(Request $request)
    {
        try {
            ...
            if ($billings->isEmpty()) {
                return response()->json([
                    'success' => false,
                    'message' => 'Inga fakturor hittades för nedladdning.',
                ], 404);
            }
            ...
            foreach ($billings as $billing) {
                $relativeFilePath = ltrim((string) ($billing->file ?? ''), '/');
                ...
                if (isset($usedEntryNames[$entryName]))
                    $entryName = $billing->id.'_'.$entryName;
            ...
            return response()->download($zipPath, 'billings.zip', [
