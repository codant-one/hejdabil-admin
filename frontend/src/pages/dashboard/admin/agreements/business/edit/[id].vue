...
async function fetchData() {
    ...
    if(Number(route.params.id) && route.name === 'dashboard-admin-agreements-business-edit-id') {
        ...
        year.value = agreement.value.offer.year ?? year.value
        control_inspection.value = agreement.value.offer.control_inspection ?? control_inspection.value
        color.value = agreement.value.offer.color ?? color.value
        fuel_id.value = agreement.value.offer.fuel_id ?? fuel_id.value
        gearbox_id.value = agreement.value.offer.gearbox_id ?? gearbox_id.value
        chassis.value = agreement.value.offer.chassis ?? chassis.value
        engine.value = agreement.value.offer.engine ?? engine.value
        car_name.value = agreement.value.offer.car_name ?? car_name.value
        number_keys.value = agreement.value.offer.number_keys ?? number_keys.value
        service_book.value = agreement.value.offer.service_book ?? service_book.value
        summer_tire.value = agreement.value.offer.summer_tire ?? summer_tire.value
        winter_tire.value = agreement.value.offer.winter_tire ?? winter_tire.value
        last_service.value = agreement.value.offer.last_service ?? last_service.value
        last_service_date.value = agreement.value.offer.last_service_date ?? last_service_date.value
        dist_belt.value = agreement.value.offer.dist_belt ?? dist_belt.value
        last_dist_belt.value = agreement.value.offer.last_dist_belt ?? last_dist_belt.value
        last_dist_belt_date.value = agreement.value.offer.last_dist_belt_date ?? last_dist_belt_date.value
        ...
        if (agreement.value.agreement_client) {
          const resolvedCountryId = agreement.value.agreement_client.client_type_id === 3
            ...
            : agreement.value.agreement_client.country_id ?? agreement.value.agreement_client.country?.id ?? agreement.value.agreement_client.country?.name ?? country_id.value
...
const selectClient = client => {
  ...
  landline.value = normalizeLandlineForInput(selected.landline)
...
const onSubmit = async (forceSave = false) => {
  ...
  const hasTab1Errors = !organization_number.value || 
    ...
    const shouldSubmitAll = forceSave === true || Number(currentTab.value) === 1
        ...
        <VCardText class="p-0 d-flex w-100">
          ...
          <div class="d-flex mb-4" :class="windowWidth < 1024 ? 'w-100 gap-2' : 'gap-4'">
              ...
              :class="[
                ...
                currentTab !== 1 ? 'btn-light' : 'btn-gradient'
              ]"
                  @click="onSubmit()"
              >
