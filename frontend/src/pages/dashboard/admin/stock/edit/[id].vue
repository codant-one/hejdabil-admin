...
async function fetchData() {
    ...
    if(Number(route.params.id) && route.name === 'dashboard-admin-stock-edit-id') {
        ...
        const resolvedCountryId = vehicle.value.client_purchase?.client_type_id === 3
            ? vehicle.value.client_purchase?.country_id ?? vehicle.value.client_purchase?.country?.id ?? vehicle.value.client_purchase?.country?.name ?? defaultForeignCountryId
            : vehicle.value.client_purchase?.country_id ?? vehicle.value.client_purchase?.country?.id ?? vehicle.value.client_purchase?.country?.name ?? country_id.value
        ...
        phone.value = normalizePhoneForInput(vehicle.value.client_purchase?.phone ?? phone.value, vehicle.value.client_purchase?.client_type_id === 3 ? resolvedCountryId : null)
        landline.value = normalizeLandlineForInput(vehicle.value.client_purchase?.landline ?? landline.value)
...
const getTabValidationErrors = () => {
    ...
    const hasTab1Errors = purchase_price.value === null ||
...
const validateTabByIndex = async (tabIndex, tabErrors = getTabValidationErrors()) => {
    const tabMessages = {
        0: 'Vänligen fyll i alla obligatoriska fält i fliken Fordon',
        1: 'Vänligen fyll i alla obligatoriska fält i fliken Prisinformation',
...
const onSubmit = async () => {
    ...
    if (currentTab.value === 4) {
        if (hasTab0Errors) {
            ...
            await nextTick()
            refForm.value?.validate()
            showTabValidationWarning('Vänligen fyll i alla obligatoriska fält i fliken Fordon')
            return
        }
        ...
        if (hasTab1Errors) {
            ...
            await nextTick()
            refForm.value?.validate()
            showTabValidationWarning('Vänligen fyll i alla obligatoriska fält i fliken Prisinformation')
            return
        }
        ...
        if (hasTab2Errors) {
            ...
            await nextTick()
            refForm.value?.validate()
            showTabValidationWarning('Vänligen fyll i alla obligatoriska fält i fliken Kund')
            return
        }
            ...
            >
                ...
                <VCardText class="px-0">
                    <VWindow v-model="currentTab">
                            ...
                            >
                                <div :style="windowWidth < 1024 ? 'width: 100%;' : 'width: calc(50% - 12px);'">
                                    ...
                                    <VTextField
                                        ...
                                        suffix="KR"
                                        :rules="[requiredValidator, ...nonNegativeNumericRules]"
                                        @input="purchase_price = normalizeNumericTextInput(purchase_price)"
                                        @keydown="handlePhoneKeydown"
                                    />
                                </div>
                                <div :style="windowWidth < 1024 ? 'width: 100%;' : 'width: calc(50% - 12px);'">
                        ...
                        <!-- Planerade åtgärder -->
                        <VWindowItem :value="3" class="px-md-0">
                        ...
                        <!-- Dokument -->
                        <VWindowItem :value="4" class="px-md-0">
                            <div 
                            ...
                            <VRow>
                                ...
                                <VCol cols="12" md="12" class="pb-0">
                                    ...
                                    <VTextField
                                        ...
                                        suffix="KR"
                                        :rules="[requiredValidator, ...nonNegativeNumericRules]"
                                        @input="cost = normalizeNumericTextInput(cost)"
                                        @keydown="handlePhoneKeydown"
                                    />
                                </VCol>
                            ...
                            <VRow>
                                ...
                                <VCol cols="12" md="12" class="pb-0">
                                    ...
                                    <VTextField
                                        ...
                                        :rules="[requiredValidator, ...nonNegativeNumericRules]"
                                        :readonly="!isEdit"
                                        @input="selectedTask.cost = normalizeNumericTextInput(selectedTask.cost)"
                                        @keydown="handlePhoneKeydown"
                                    />
                                </VCol>
