    ...
    if (isValid) {
      ...
      profileStores
        ...
        .then((response) => {
          ...
          alert.value.message =
            "Personlig information uppdaterad. Sidan kommer automatiskt att laddas om för att observera ändringarna...!";
          alert.value.show = true;
...
<template>
  <VRow class="justify-center m-0 p-0">
    ...
    <VCol cols="12" md="6" class="d-flex flex-column pa-md-8 pa-0 bg-white">
      ...
      >
        <template v-if="role === 'Supplier' || role === 'User'">
          ...
          <VWindow
            v-model="controlledTab"
            class="disable-tab-transition mt-8"
            :touch="false"
          >
            <VWindowItem>
        ...
        <template v-else>
          ...
          <!-- 👉 Form Actions -->
          <div>
            <VBtn type="submit" class="w-100 w-md-auto"> Kom igång </VBtn>
          </div>
        </template>
      </VForm>
    </VCol>
